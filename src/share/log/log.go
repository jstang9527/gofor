...
func Init(srvName string, Pro bool) *zap.Logger {
	instance = NewLogger(srvName, Pro)
	return instance
}

// NewLogger 新建日志
func NewLogger(srvName string, Pro bool) *zap.Logger {
	directory := config.LogPath //"D:\\Tools\\Go_Work\\src\\my-micro\\"
	if len(directory) == 0 {
		directory = path.Join("..", "logs", srvName)
	} else {
		directory = path.Join(directory, srvName)
	}
	writers := []zapcore.WriteSyncer{newRollingFile(directory)}
	writers = append(writers, os.Stdout)
	logger, _ := newZapLogger(Pro, zapcore.NewMultiWriteSyncer(writers...))
